...
coords = dict(loadmat('cornercoordinates.mat')) # replace me
##--your-code-ends-here--##

# Load image coordinates for the corners from coords-dictionary.

#--your-code-starts-here--##
x1 = coords["x1"] # replace me
x2 = coords["x2"] # replace me 
y1 = coords["y1"] # replace me 
y2 = coords["y2"] # replace me 
#--your-code-ends-here--##
...
##-your-code-starts-here-##
proj1= np.dot(P1t, np.vstack((ABCDEFGH_w.T,np.ones(8))))
proj2= np.dot(P2t, np.vstack((ABCDEFGH_w.T,np.ones(8))))
cx1 = (proj1[0,:] / proj1[2,:]) # replace me
cy1 = (proj1[1,:] / proj1[2,:])  # replace me
cx2 = (proj2[0,:] / proj2[2,:])  # replace me
cy2 = (proj2[1,:] / proj2[2,:])   # replace me
...
##-your-code-starts-here-##
proj_c1= np.dot(P1, np.vstack((Xc,np.ones(8))))
proj_c2= np.dot(P2, np.vstack((Xc,np.ones(8))))
pcx1 = (proj_c1[0,:] / proj_c1[2,:])  # replace me
pcy1 = (proj_c1[1,:] / proj_c1[2,:])  # replace me
pcx2 = (proj_c2[0,:] / proj_c2[2,:]) # replace me
pcy2 = (proj_c2[1,:] / proj_c2[2,:])  # replace me
##-your-code-starts-here-##

# pcx1 and pcy1 are x and y coordinates for image 1, and similarly for image 2
plt.figure(1, figsize=(20,15), dpi=200)
