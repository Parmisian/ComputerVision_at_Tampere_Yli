    ...
    try:            
        ...
        if not ret_val:
            ...
            if len(p0) <= 10:
                ...
                ##-your-code-starts-here-##
                #isFound=isFound.flatten()
                
                idx= [i for i, x in enumerate(isFound) if x]
                good_new = p1[idx]
                good_old = p0[idx]
                #also for p0-- just in case we want to track the motion
                ##-your-code-starts-here-##
                ...
                for i,(new,old) in enumerate(zip(good_new,good_old)):
                    a,b = new.ravel()
                    c,d = old.ravel()
                    cv2.drawMarker(img, (int(a),int(b)), color=(0,255,0), markerType=cv2.MARKER_CROSS, thickness=2)
                ##-your-code-ends-here-##
