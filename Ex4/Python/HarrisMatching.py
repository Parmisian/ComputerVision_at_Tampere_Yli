...
for i1 in range(npts1):
    for i2 in range(npts2):
        ...
        ghat= g.mean()
        fhat= f.mean()
        nom= np.sum((g-ghat)*(f-fhat))
        denom= np.sqrt(np.sum((g-ghat)**2)*np.sum((f-fhat)**2))
        distmat[i1,i2] = nom/denom
...
id_ncc = np.argsort(-pairs[:,2], axis=0) #note the - signs for descending order
##-your-code-ends-here-##
...
#how to find the no of correct matches?? NCC