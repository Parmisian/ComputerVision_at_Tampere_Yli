...
while N > sample_count:
    ...
    ##-your-code-starts-here-##
    inliers_ix= np.abs(np.dot(l,points_h)) <t
    inliers= np.where(inliers_ix)# indexes when the cond is true(or np.nonzero()[0])
    ##-your-code-ends-here-##
...
##-your-code-starts-here-##
inlier_ix= np.abs(np.dot(l,points_h)) <t
inliers= np.nonzero(inlier_ix)[0] # indexes, not points themselves
x_inliers= x[inliers]
y_inliers= y[inliers]
##-your-code-ends-here-##  
