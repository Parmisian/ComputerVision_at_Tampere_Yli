...
# How many of the top 5 matches appear to be correct correspondences? 2 or 3
...
for i in range(pairs.shape[0]):
    target = int(pairs[i, 0])
    sorted_neigh = distmat_sorted[target,:]
    d_1 = sorted_neigh[0] 
    d_2 = sorted_neigh[1]
    nddr_i = d_1/d_2
    
    #pairs_nddr[i] = [pairs[i, 0], pairs[i, 1], nddr_i]
    nndr[i] = nddr_i
    
# We sort the mutually nearest neighbors based on the distance 
snnd = np.sort(nndr, axis=0)
